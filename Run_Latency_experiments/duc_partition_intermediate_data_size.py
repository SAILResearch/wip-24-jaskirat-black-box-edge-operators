...
# Load the ONNX model
model = onnx.load('ResNet101-DUC-12.onnx')
...
for node in model.graph.node:
    ...
    if node.name=='conv5_1_1x1_proj/bn':
        node_outputs = node.output
        # Run the ONNX model for the current node's outputs
        for image_tensor in image_tensors:
            ort_outs = ort_session.run(node_outputs, {input_name: image_tensor})
...
csv_file_path = "duc_partition_intermediate_data_size.csv"
