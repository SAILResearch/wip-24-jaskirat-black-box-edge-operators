...
import time
dataset= ['ILSVRC2012_val_00033687.JPEG']
                ...
                if single_tier == 'mobile' or single_tier == 'edge':
                    for mobile_edge_bandwidth in mobile_edge_bandwidths:
                        ...
                        for i in range(6):
                            ...
                            for img in dataset:
                                with open(os.path.join('imagenet', img), "rb") as image_file:
    ...
    if operator =="sptq":
        for subject in subjects:
            ...
            for multi_tier in multi_tiers:
                ...
                if multi_tier == 'mobile_edge':
                    for mobile_edge_bandwidth in mobile_edge_bandwidths:
                        url, result_file=construct_url_for_sptq_partition(multi_tier, subject, mobile_edge_bandwidth, '')
                        with open(result_file,'w') as fd:
                            fd.write('Experiment Results\n')
                        for i in range(6):
                            print("Experiment:",i+1)
