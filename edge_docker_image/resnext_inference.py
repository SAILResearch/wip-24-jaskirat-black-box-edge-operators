...
def resnext_single_inference(img):
    ...
    session = ort.InferenceSession(
        'resnext_models/resnext101.onnx', providers=['CPUExecutionProvider'])
    result = session.run(
...
def resnext_earlyexit_single_inference(img):
    ...
    input_data = resnext_model.preprocess(image)
    session = ort.InferenceSession(
        'resnext_models/resnext101_earlyexit.onnx', providers=['CPUExecutionProvider'])
    result = session.run(
        [session.get_outputs()[0].name], {session.get_inputs()[0].name: input_data})
    result = resnext_model.postprocess(result)
    return result
...
def resnext_int8_sptq_earlyexit_single_inference(img):
    ...
    input_data = resnext_model.preprocess(image)
    session = ort.InferenceSession(
        'resnext_models/resnext101_int8_sptq_earlyexit.onnx', providers=['CPUExecutionProvider'])
    result = session.run(
        [session.get_outputs()[0].name], {session.get_inputs()[0].name: input_data})
    result = resnext_model.postprocess(result)
    return result
...
def resnext_split_first_half_single_inference(img):
    ...
    input_data = resnext_model.preprocess(image)
    session = ort.InferenceSession(
        'resnext_models/resnext101_split_part1.onnx', providers=['CPUExecutionProvider'])
...
def resnext_split_second_half_single_inference(img):
    ...
    # get the name of the first input of the model
    session = ort.InferenceSession(
        'resnext_models/resnext101_split_part2.onnx', providers=['CPUExecutionProvider'])
    result = session.run([session.get_outputs()[0].name], {
...
def resnext_int8_sptq_earlyexit_first_half_single_inference(img):
    ...
    input_data = resnext_model.preprocess(image)
    session = ort.InferenceSession(
        'resnext_models/resnext101_int8_sptq_earlyexit_part1.onnx', providers=['CPUExecutionProvider'])
    result = session.run([session.get_outputs()[0].name], {
        session.get_inputs()[0].name: input_data})
    return result
...
def resnext_int8_sptq_earlyexit_second_half_single_inference(img):
    ...
    # get the name of the first input of the model
    session = ort.InferenceSession(
        'resnext_models/resnext101_int8_sptq_earlyexit_part2.onnx', providers=['CPUExecutionProvider'])
    result = session.run([session.get_outputs()[0].name], {
        session.get_inputs()[0].name: img[0]})
    result = resnext_model.postprocess(result)
    return result
...
def resnext_int8_sptq_single_inference(img):
    ...
    input_data = resnext_model.preprocess(image)
    session = ort.InferenceSession(
        'resnext_models/resnext101_int8_sptq.onnx', providers=['CPUExecutionProvider'])
    result = session.run(
        [session.get_outputs()[0].name], {session.get_inputs()[0].name: input_data})
    result = resnext_model.postprocess(result)
    return result
