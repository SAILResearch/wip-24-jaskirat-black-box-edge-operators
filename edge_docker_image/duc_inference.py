...
def duc_single_inference(img):
    ...
    session = ort.InferenceSession(
        'duc_models/ResNet101-DUC-12.onnx', providers=['CPUExecutionProvider'])
    result = session.run(
...
def duc_earlyexit_single_inference(img):
    ...
    input_data = duc_model.preprocess(image, rgb_mean)
    session = ort.InferenceSession(
        'duc_models/ResNet101-DUC-12_earlyexit.onnx', providers=['CPUExecutionProvider'])
    result = session.run(
        [session.get_outputs()[0].name], {session.get_inputs()[0].name: input_data})
    result = duc_model.postprocess(result[0][0], result_shape)
    return result
...
def duc_int8_sptq_earlyexit_single_inference(img):
    ...
    input_data = duc_model.preprocess(image, rgb_mean)
    session = ort.InferenceSession(
        'duc_models/ResNet101-DUC-12_int8_sptq_earlyexit.onnx', providers=['CPUExecutionProvider'])
    result = session.run(
        [session.get_outputs()[0].name], {session.get_inputs()[0].name: input_data})
    result = duc_model.postprocess(result[0][0], result_shape)
    return result
...
def duc_split_first_half_single_inference(img):
    ...
    input_data = duc_model.preprocess(image, rgb_mean)
    session = ort.InferenceSession(
        'duc_models/ResNet101-DUC-12_split_part1.onnx', providers=['CPUExecutionProvider'])
...
def duc_split_second_half_single_inference(img):
    ...
    session = ort.InferenceSession(
        'duc_models/ResNet101-DUC-12_split_part2.onnx', providers=['CPUExecutionProvider'])
...
def duc_int8_sptq_earlyexit_first_half_single_inference(img):
    ...
    input_data = duc_model.preprocess(image, rgb_mean)
    session = ort.InferenceSession(
        'duc_models/ResNet101-DUC-12_int8_sptq_earlyexit_part1.onnx', providers=['CPUExecutionProvider'])
...
def duc_int8_sptq_earlyexit_second_half_single_inference(img):
    ...
    session = ort.InferenceSession(
        'duc_models/ResNet101-DUC-12_int8_sptq_earlyexit_part2.onnx', providers=['CPUExecutionProvider'])
    result = session.run([session.get_outputs()[0].name], {session.get_inputs(
...
def duc_int8_sptq_single_inference(img):
    ...
    input_data = duc_model.preprocess(image, rgb_mean)
    session = ort.InferenceSession(
        'duc_models/ResNet101-DUC-12_int8_sptq.onnx', providers=['CPUExecutionProvider'])
    result = session.run(
        [session.get_outputs()[0].name], {session.get_inputs()[0].name: input_data})
    result = duc_model.postprocess(result[0][0], result_shape)
    return result
